        ...
        for n in nodes:
            ...
            if eur_cents and cards and str(cards[0].get('rarityTyped')).lower() == 'rare':
                ...
                if f_rare is not None and price_now <= f_rare:
                    ...
                    # Construction de l'URL de la carte spécifique
...
# --- UI ---
st.title("🎯 Sniper Pro : Undercuts avec Liens Directs")
...
if st.session_state.get('token'):
    if st.button("🚀 Lancer le Scan"):
        ...
        if data:
            ...
            # Utilisation de column_config pour rendre l'URL cliquable et jolie
            st.dataframe(
                df.style.apply(style_df, axis=1),
                column_config={
                    "Lien": st.column_config.LinkColumn(
                        "Acheter", 
                        help="Lien direct vers la carte sur Sorare",
                        validate="^https://sorare\\.com/.*",
                        display_text="🛒" # Affiche une icône caddie au lieu de l'URL moche
                    ),
                },
            ...
            st.warning("Aucun undercut détecté.")
